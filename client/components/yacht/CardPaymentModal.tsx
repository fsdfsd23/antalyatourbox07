...
export interface CardPaymentModalProps {
  ...
  guest: { fullName: string; email: string; phone: string; time?: string } | null;
}

export default function CardPaymentModal({ open, onOpenChange, onPaid, guest }: CardPaymentModalProps) {
  ...
  const valid = holder.trim() && number.replace(/\s+/g, "").length >= 12 && /^(0[1-9]|1[0-2])\/(\d{2})$/.test(expiry) && cvc.length >= 3;
          ...
          <div className="space-y-4">
            <div className="h-40 perspective-1000">
              <motion.div
                ...
                style={{ transformStyle: "preserve-3d", background: "linear-gradient(135deg,#ef4444,#dc2626)" }}
              >
                ...
                <div className="absolute inset-0 p-4" style={{ transform: "rotateY(180deg)", backfaceVisibility: "hidden" }}>
                  <div className="w-full h-8 bg-black/70 mt-6" />
                  <div className="mt-6 text-right">
                    <div className="inline-block bg-white px-2 py-1 text-black rounded">{cvc || "•••"}</div>
                  </div>
                </div>
              </motion.div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <Input placeholder="CVC" value={cvc} onChange={(e) => setCvc(e.target.value)} onFocus={() => setFocusBack(true)} onBlur={() => setFocusBack(false)} />
            </div>
            <div className="flex justify-end">
              <Button disabled={!valid} onClick={() => onPaid(holder)}>Ödemeyi Tamamla</Button>
            </div>
          </div>
          <ReservationSummary fullName={guest?.fullName} email={guest?.email} phone={guest?.phone} time={guest?.time} />
        </div>
      </DialogContent>
    </Dialog>
  );
