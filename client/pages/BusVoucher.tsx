...
export default function BusVoucher() {
  ...
  const code = q.get("code") || Math.random().toString(36).toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 8);
  ...
  const distanceKm = (() => {
    const a = CITY_COORDS[from] || CITY_COORDS["İstanbul"]; const b = CITY_COORDS[to] || CITY_COORDS["İzmir"]; return haversineKm(a, b);
  })();
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <div className="text-lg space-y-2">
          <Card className="p-4 space-y-1">
            ...
            <div>E-posta: <strong>{email}</strong></div>
          ...
          <Card className="p-4 space-y-1">
            ...
            <div>Mesafe: <strong>{distanceKm.toFixed(1)} km</strong></div>
          </Card>
        </div>
      </div>

      <BusRouteMap from={CITY_COORDS[from] || CITY_COORDS["İstanbul"]} to={CITY_COORDS[to] || CITY_COORDS["İzmir"]} />
    </div>
  );
}
