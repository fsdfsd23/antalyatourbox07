    ...
    <div className="container max-w-6xl container-px py-6 grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6">
      <main className="space-y-4">
        ...
        <div className="flex justify-end">
          <Button
            className="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white text-lg font-extrabold px-6 py-6"
            disabled={!valid}
            onClick={() => {
              ...
              const seats = selected
                ...
                .join(",");
              navigate(
                `/otobus/voucher?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&date=${date}&trip=${trip.id}&seats=${seats}&code=${code}&name=${encodeURIComponent(name)}&email=${encodeURIComponent(email)}&phone=${encodeURIComponent(phone)}`,
              );
            }}
          >
            Güvenli Ödeme Yap • ₺ {subtotal.toLocaleString("tr-TR")}
          </Button>
        </div>
      </main>
      <aside className="space-y-3">
        <Card className="p-4 text-sm space-y-1">
          ...
          <div className="font-bold">
            Tutar: ₺ {subtotal.toLocaleString("tr-TR")}
          </div>
        </Card>
      </aside>
    </div>
